...
void CDriverWizardAppWiz::CustomizeProject(IBuildProject* pProject)
{
	...
	for(int i=1; i <= Count; i++)
	{
		...
		pConfig->AddToolSettings(L"cl.exe", L"/I \"$(ddkroot)\\inc\\ddk\\wxp\"", dummy);
		pConfig->AddToolSettings(L"cl.exe", L"/I \"$(ddkroot)\\inc\\wxp\"", dummy);
		pConfig->AddToolSettings(L"cl.exe", L"/I \"$(ddkroot)\\inc\\crt\"", dummy);
...
//		pConfig->AddToolSettings(L"link.exe", L"int64.lib", dummy);
		...
		pConfig->AddToolSettings(L"link.exe", L"/libpath:\"$(ddkroot)\\lib\\wxp\\i386\"", dummy);
